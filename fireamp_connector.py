...
class FireAMPConnector(BaseConnector):
    ...
    def _make_rest_call(self, endpoint, is_absolute_url=False, method="get", params=None, data=None):
        url = "{0}{1}".format(self._base_url, endpoint) if not is_absolute_url else endpoint
    ...
    def handle_page_of_results(self, resp_json:dict):
        ...
        for item in resp_json["data"]:
            ...
            artifacts = []
            container = {'name': f"FireAMP Container - {time.time()}"}
            artifact = item.copy()
            artifacts.append({'name': 'FireAMP Artifact', 'cef': artifact})
            source_data_identifier = hashlib.sha256(result.encode('utf-8')).hexdigest()
            container['artifacts'] = artifacts
            container['source_data_identifier'] = source_data_identifier
            return_value, msg, cid = self.save_container(container)
    ...
    def _handle_on_poll(self, param):
        ...
        while True:
            ...
            if phantom.is_fail(ret_val):
                return action_result.set_status(phantom.APP_ERROR, r_json)
            ...
            if "next" not in r_json["metadata"]["links"]:
                ...
                target_url = r_json["metadata"]["links"]["next"]
    ...
    def handle_action(self, param):
       ...
       #     ret_val = self._handle_on_poll(param)
        ...
        if action == phantom.ACTION_ID_TEST_ASSET_CONNECTIVITY:
          ...
          # chamge  
        elif (action == self.ACTION_ID_FORWARD_LOOKUP):
            ret_val = self._handle_forward_lookup(param)
        elif (action == self.ACTION_ID_REVERSE_LOOKUP):
